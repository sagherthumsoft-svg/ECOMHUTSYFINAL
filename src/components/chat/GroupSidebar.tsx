// Fix 7 — Auth Wrapper: GroupSidebar uses onAuthStateChanged so that
// Firestore queries never run before Firebase Auth is ready.
// Unread: uses localStorage timestamps to track per-group read state.
"use client";
...
export default function GroupSidebar() {
  ...
  // Helper: is this group unread?
  const isUnread = useCallback(
    (group: Group & { chatId?: string; updatedAt?: number }) => {
      if (!dbUser?.uid) return false;
      const chatId = group.chatId || group.groupId;
      const lastSenderId = (group as any).lastMessageSenderId || "";
      if (!lastSenderId || lastSenderId === dbUser.uid) return false;
      const lastMsgAt: number = group.lastMessageAt || (group as any).updatedAt || 0;
    ...
    const unsubAuth = onAuthStateChanged(auth, (firebaseUser) => {
      ...
      const unsubSnap = onSnapshot(
        ...
        (snapshot) => {
          const fetched = snapshot.docs
            .map((d) => {
              ...
              return {
                groupId: d.id, // mapped to id for activeGroup compatibility
                chatId: d.id,
                name: data.name || data.groupName || "Unnamed Group",
                ...
                type: "group",
              } as Group & { chatId: string; updatedAt: number; type: string };
            })
            .filter(Boolean) as (Group & { chatId: string; updatedAt: number; type: string })[];
        ...
        (err) => {
          ...
          setError("Could not load groups, please refresh.");
    ...
    const handleOpenChat = (e: any) => {
      ...
      if (type === "group" && id) {
        ...
        if (group) {
          ...
          markChatRead((group as any).chatId || group.groupId);
          setReadTick((t) => t + 1);
  ...
  }, [groups, setActiveGroup, setReadTick]);
  ...
  return (
    <div className="flex flex-col w-full h-full bg-[#111B21] border-r border-[#2A3942] relative">
      <style>{`
        .group-sidebar-scroll::-webkit-scrollbar { width: 5px; }
        .group-sidebar-scroll::-webkit-scrollbar-track { background: transparent; }
        .group-sidebar-scroll::-webkit-scrollbar-thumb { background-color: #2A3942; border-radius: 10px; }
      `}</style>
      ...
      {/* Floating New Group Button (only for admin roles) */}
      {canCreate && (
        <button
      ...
      {/* Content area */}
      <div className="flex-1 overflow-y-auto w-full group-sidebar-scroll">
        {/* Loading */}
        {isLoading && (
        ...
        {!isLoading && error && (
          <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center">
            ...
            <p className="text-sm text-red-400">Could not load groups.</p>
        ...
        {!isLoading && !error && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center text-[#8696A0] mt-10">
            <UsersIcon className="w-12 h-12 opacity-20 mb-2" />
            <p className="text-sm">
        ...
        {!isLoading &&
          !error &&
          filtered.map((group) => {
            const time = group.lastMessageAt || group.createdAt;
            const isActive = activeGroup?.groupId === group.groupId;
            const chatId = (group as any).chatId || group.groupId;
            const unread = isUnread(group as any);
