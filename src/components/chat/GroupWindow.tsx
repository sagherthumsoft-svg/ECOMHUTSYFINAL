...
export default function GroupWindow() {
  const { activeGroup, setActiveGroup, messagesCache, setMessagesCache } = useGroupStore();
  ...
  useEffect(() => {
    ...
    // 2. Enrollment check
    if (!activeGroup.memberIds.includes(uid)) {
      if (isAdmin) {
        const enroll = async () => {
          try {
    ...
    return () => unsubSnap();
  }, [activeGroup?.groupId, dbUser?.uid]);

  const handleSend = async () => {
    const groupId = activeGroup?.groupId;
    const uid = dbUser?.uid;
    if (!text.trim() || !groupId || !uid) return;
    ...
    try {
      ...
      const parts = (activeGroup?.memberIds || []).filter((p: string) => p !== uid);
      await Promise.all(
        parts.map(async (partUid: string) => {
          await addDoc(collection(db, "clientNotifications"), {
            receiverIds: [partUid],
            type: "New Group Message",
            message: `${activeGroup?.name} - ${dbUser?.name || "User"}: ${messageText.substring(0, 50)}${messageText.length > 50 ? "..." : ""}`,
            isRead: false,
            createdAt: Date.now(),
  ...
  const handleFileUpload = async (
    ...
    if (!file || !activeGroup || !dbUser?.uid) return;
    ...
    try {
      ...
      const path = `storage/chats/${groupId}/files/${Date.now()}_${actualFile.name}`;
      ...
      uploadTask.on(
        ...
        async () => {
          try {
            ...
            await addDoc(collection(db, "chats", groupId, "messages"), newMsg);
            ...
            await Promise.all(
              parts.map(async (partUid: string) => {
                await addDoc(collection(db, "clientNotifications"), {
                  receiverIds: [partUid],
                  type: "New Group Media",
                  message: `${activeGroup.name} - ${dbUser?.name || "User"} sent an attachment.`,
                  isRead: false,
                  createdAt: Date.now(),
                  link: "/dashboard/groups",
                  groupId: groupId,
  ...
  const handleDeleteMessage = async (msgToDelete: Message) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, "chats", groupId, "messages", msgDocId));
  ...
  const handleClearChat = async () => {
    ...
    try {
      await Promise.all(
        messages.map((msg) =>
          deleteDoc(doc(db, "chats", groupId, "messages", msg.messageId || (msg as any).id))
        )
      );
      ...
      {rightPanelMode && (
        <RightPanel
          ...
          chatData={{
            ...
            membersCount: activeGroup?.memberIds?.length || 0,
