...
export default function CreateWarehouseModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const selectedStaffNames = users
    .filter(u => staff.includes(u.uid))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (staff.length === 0) return toast.error("Please add at least one staff member");
    
    setLoading(true);
    try {
      const currentUser = auth.currentUser;
      if (!currentUser) throw new Error("You must be logged in to create a warehouse.");
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white dark:bg-[#111b21] rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-200 dark:border-zinc-800">
          <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-zinc-800 shrink-0">
            <div>
              <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Create Warehouse</h2>
              <p className="text-xs text-slate-500 mt-1">Register a new storage location</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider ml-1">Warehouse Name</label>
              <input 
                type="text" 
                value={name}
            ...
            <div className="space-y-1.5">
              ...
              <textarea 
                ...
                placeholder="Provide a description for this warehouse"
              />
            </div>

            <div className="space-y-3">
              <div className="flex justify-between items-center px-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Assigned Staff</label>
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-bold text-sky-600 bg-sky-50 dark:bg-sky-900/20 px-2.5 py-0.5 rounded-full border border-sky-100 dark:border-sky-800/50">
                    {staff.length} Staff
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-3.5 bg-sky-50/50 dark:bg-sky-900/5 border-2 border-dashed border-sky-200/50 dark:border-sky-800/30 rounded-2xl text-sky-600 dark:text-sky-400 font-bold hover:bg-sky-50 dark:hover:bg-sky-900/10 transition-all group"
              >
              ...
              {staff.length > 0 && (
                <div className="flex flex-wrap gap-2 p-3 bg-slate-50 dark:bg-zinc-800/50 rounded-xl border border-slate-100 dark:border-zinc-800 max-h-32 overflow-y-auto">
                  {selectedStaffNames.map((userName, i) => (
